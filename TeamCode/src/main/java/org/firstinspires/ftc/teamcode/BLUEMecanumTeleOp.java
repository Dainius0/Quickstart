...
public class BLUEMecanumTeleOp extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            // Turret control - only update every N loops (BLUE SIDE: mirror RED side angles)
            if (loopCounter % TURRET_UPDATE_INTERVAL == 0) {
                double targetAngle;
                if (turretTrackingEnabled) {
                    // BLUE SIDE: Robot starts at 180°, so we need to subtract 180° from heading
                    // Then negate the angle to mirror RED side
